...
public static class WeaponsPatcher
{
    ...
    private static void AddBreakdownRecipe(IWeaponGetter weapon, bool noRecipes = false)
    {
        ...
        if (!noRecipes)
        {
            foreach (var recipe in Executor.AllRecipes!)
            {
                PatchingData.Log.Info($"Existing skipping: {Settings.General.SkipExisting}, weapon correct: {recipe.Items?.FirstOrDefault()?.Item.Item.FormKey}, is smelter: {recipe.WorkbenchKeyword.FormKey == "CraftingSmelter".GetFormKey()} )");
