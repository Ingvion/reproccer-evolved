...
static class Executor
{
    ...
    internal static Lazy<Settings.AllSettings> PatcherSettings = new();
    internal static Settings.AllSettings? Settings;

    internal static List<IConstructibleObjectGetter>? AllRecipes;
    internal static bool IsSplitted = false;        // True if potential master files overflow were detected
    internal static HashSet<FormKey> Session = [];  // list of processed forms
    ...
    public static async Task<int> Main(string[] args)
    {
        return await SynthesisPipeline.Instance
            ...
            .SetAutogeneratedSettings("Settings", "Settings.json", out PatcherSettings)
    ...
    {
        Settings = PatcherSettings.Value;
        State = patcherState;
        ...
        var formKeyCache = new TextFileFormKeyAllocator(State.PatchMod, $"{userDir}formsCache");
        State.PatchMod.SetAllocator(formKeyCache);

        // running patchers
        if (Settings.General.ArmorPatcher) 
            Patchers.ArmorPatcher.Run();
        if (Settings.General.WeaponsPatcher)
            Patchers.WeaponsPatcher.Run();
        if (Settings.General.ProjectilesPatcher) 
            Patchers.ProjectilesPatcher.Run();
        if (Settings.General.IngredientsPatcher) 
            Patchers.IngredientsPatcher.Run();

        if (IsSplitted)
        {
            Console.WriteLine($"\n~> Patch file requires too many master files and will be split into several parts.\n" +
                $"~> Rename the generated file and run ReProccer Evolved again to proceed.\n");

            File.WriteAllLines($"{userDir}/session", Session.Select(formKey => formKey.ToString()));
        }
        else
        {
            Console.WriteLine($"\n~> Patching completed!\n");

            File.WriteAllText($"{userDir}/session", string.Empty);
        }

        formKeyCache.Commit();

        State.PatchMod.ModHeader.Description = $"Generated by ReProccer Evolved 1.01 ({DateTime.Now})";
    ...
    /// <summary>
    /// Loading session file.<br/>
    /// </summary>
    private static void LoadSession(string userPath)
    {
        if (!File.Exists($"{userPath}/session") || new FileInfo($"{userPath}/session").Length == 0)
            return;

        foreach (var entry in File.ReadLines($"{userPath}/session"))
        {
            if (string.IsNullOrWhiteSpace(entry)) continue;

            string[] data = entry.Split(':');
            Session.Add(new(data[1], Convert.ToUInt32(data[0], 16)));
        }
    }
    ...
    private static JsonObject MergeRules(string userPath)
    {
        ...
        if (!loadOrder.Any(file => file.Key.FileName.Equals("Skyrim AE Redone - Main.esp")))
            throw new InvalidLOException("\n\n--> Records patching is only required if the Main Module is installed.\n");
